...
function PWManagement() {
  ...
  useEffect(() => {
    ...
    axios.get('https://joyous-ocean-production.up.railway.app/api/teeth')
        .then(response => {
  ...
  const removePWFromGroup = async () => {
    try {
      ...
      await axios.delete('https://joyous-ocean-production.up.railway.app/api/groupes/${selectedGroupId}/remove-pw/${selectedPWId}');
