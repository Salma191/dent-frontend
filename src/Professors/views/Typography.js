...
function Typography() {
  ...
  const fetchTeeth = async () => {
    try {
      const response = await axios.get('https://joyous-ocean-production.up.railway.app/api/teeth');
  ...
  const createOrUpdateTooth = async () => {
    try {
      if (toothFormData.id) {
        // Update tooth
        await axios.put(`https://joyous-ocean-production.up.railway.app/api/teeth/${toothFormData.id}`, toothFormData);
      } else {
        // Create new tooth
        await axios.post('https://joyous-ocean-production.up.railway.app/api/teeth', toothFormData);
      }
  ...
  const deleteTooth = async (id) => {
    try {
      await axios.delete(`https://joyous-ocean-production.up.railway.app/api/teeth/${id}`);
      fetchTeeth();
    } catch (error) {
